...
import pro.civitaspo.embulk.input.union.loader.ReuseOutputLocalExecutorPlugin.PluginTask
...
case class ReuseOutputLocalExecutorPlugin(outputPlugin: OutputPlugin) extends ExecutorPlugin {
  private val logger: Logger = LoggerFactory.getLogger(classOf[ReuseOutputLocalExecutorPlugin])
  ...
  override def transaction(
    ...
    logger.info(s"Using local thread executor with max_threads=$maxThreads / tasks=$inputTaskCount")
    Using.resource(new ReuseOutputLocalExecutor(outputPlugin, maxThreads, inputTaskCount)) { exec =>
